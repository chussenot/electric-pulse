...
static void test_preset_initialization(void)
{
    ...
    check_int("preset_count", count, ELECTRIC_PULSE_PRESET_COUNT);
    check_true("preset_ptr", presets != NULL);
    check_true("preset_name_bass",
               strcmp(audio_builtin_instrument_preset_name(ELECTRIC_PULSE_PRESET_BASS_PULSE), "electric_pulse_bass_pulse") == 0);
    check_true("preset_name_snare",
               strcmp(audio_builtin_instrument_preset_name(ELECTRIC_PULSE_PRESET_NOISE_SNARE), "electric_pulse_noise_snare") == 0);
    check_true("preset_has_adsr", presets[ELECTRIC_PULSE_PRESET_BASS_PULSE].envelope.attack_ms >= 0);
    check_true("preset_gate_valid",
               presets[ELECTRIC_PULSE_PRESET_DARK_ARP].envelope.gate_percent > 0 &&
               presets[ELECTRIC_PULSE_PRESET_DARK_ARP].envelope.gate_percent <= 100);
}
...
static void test_builtin_timeline(void)
{
    ...
    check_int("builtin_track4_instrument",
              song->patterns[0].tracks[3].instrument, ELECTRIC_PULSE_PRESET_HAT);
