...
We benchmarked options against Electric Pulse constraints (GUI-first change, cross-platform behavior, low maintenance risk, deterministic render path unchanged).
...
- Scope fit: this problem is in the Rust GUI runtime (`src/playback.rs`), so a Rust backend keeps changes local.
...
1. Add a backend abstraction in `src/playback.rs` (`ExternalCommandBackend` + `CpalBackend`).
