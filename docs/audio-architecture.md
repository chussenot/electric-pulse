...
Electric Pulse now renders built-in songs and ABC tracks through the same portable engine path.

## Public API

Declared in `src/electric_pulse.h`:
...
```mermaid
flowchart TD
  Consumers[GUI / CLI tools / MCP / jam] --> Engine[Audio Engine API]
