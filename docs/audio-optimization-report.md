...
- Shared DSP moved into isolated portable module consumed by the audio engine and `abc.c`.
...
- Both run as part of `mise run test`. Neither requires external dependencies.

## Benchmarks
Environment: Linux native build, `-O2`, `bin/bench-audio` (`mise run bench-audio`).
...
| Target | Action |
|---|---|
| `mise run test` | Runs all tests including `test-audio` and `test-abc` |
| `mise run test-audio` | Builds and runs `bin/test-audio-dsp` (DSP regression) |
| `mise run test-abc` | Builds and runs `bin/test-abc` (ABC parser + PCM) |
| `mise run bench-audio` | Builds and runs `bin/bench-audio` (microbenchmark) |
