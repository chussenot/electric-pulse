...
pub struct ElectricPulseGuiApp {
...
impl Default for ElectricPulseGuiApp {
    fn default() -> Self {
...
impl ElectricPulseGuiApp {
    ...
    fn default_user_song_root() -> PathBuf {
        if let Some(path) = env::var_os("ELECTRIC_PULSE_USER_SONG_DIR") {
...
impl Drop for ElectricPulseGuiApp {
    fn drop(&mut self) {
        let _ = self.playback.stop();
    }
}

impl eframe::App for ElectricPulseGuiApp {
...
impl BootOptions {
    fn from_env() -> Self {
        Self {
            demo_key: env::var("ELECTRIC_PULSE_GUI_BOOT_DEMO").ok(),
            auto_render: env_flag("ELECTRIC_PULSE_GUI_BOOT_RENDER"),
            focus: match env::var("ELECTRIC_PULSE_GUI_BOOT_FOCUS").ok().as_deref() {
            ...
            editable_source: env::var("ELECTRIC_PULSE_GUI_BOOT_EDITABLE")
                .ok()
                .and_then(|value| {
            ...
            mode: match env::var("ELECTRIC_PULSE_GUI_BOOT_MODE").ok().as_deref() {
                Some("browser") => Some(EditorMode::Browser),
                Some("edit") => Some(EditorMode::Edit),
                Some("preview") => Some(EditorMode::Preview),
                _ => None,
            },
            apply_pattern_edits: env_flag("ELECTRIC_PULSE_GUI_BOOT_PATTERN_EDITS"),
            dialog: env::var("ELECTRIC_PULSE_GUI_BOOT_DIALOG")
                .ok()
                .and_then(|value| {
                    let normalized = value.to_lowercase();
...
mod tests {
    ...
    fn install_dummy_render(app: &mut ElectricPulseGuiApp) {
    ...
    fn save_after_edit_clears_dirty_state() {
        let mut app = ElectricPulseGuiApp::default();
        app.user_song_root = std::env::temp_dir().join("electric-pulse-gui-save-after-edit");
        app.create_new_song();
        app.toggle_selected_step();
        assert!(app.effective_song_dirty());
    ...
    fn open_song_roundtrip_from_saved_file() {
        let mut app = ElectricPulseGuiApp::default();
        app.user_song_root = std::env::temp_dir().join("electric-pulse-gui-roundtrip");
        app.create_new_song();
        if let Some(song) = app.editable_song.as_mut() {
    ...
    fn invalid_save_path_surfaces_error() {
        let mut app = ElectricPulseGuiApp::default();
        app.create_new_song();
        app.editor_open_path = "/dev/null/electric-pulse-invalid.abc".to_string();
